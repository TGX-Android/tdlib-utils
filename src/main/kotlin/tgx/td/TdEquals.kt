...
@OptIn(ExperimentalContracts::class)
fun MessageContent?.contentEquals(other: MessageContent?): Boolean = this.safeEqualsTo(other) { b ->
  when (this.constructor) {
    // PollOption.media:
    // messageAnimation, messageLocation, messagePhoto, messageSticker, messageVenue, or messageVideo without caption.
    // PollTypeQuiz.explanationMedia:
    // messageAnimation, messageAudio, messageDocument, messageLocation, messagePhoto, messageVenue, or messageVideo without caption.
    MessageAnimation.CONSTRUCTOR -> {
      require(this is MessageAnimation && b is MessageAnimation)
      this.animation.animation.equalsTo(b.animation.animation) && this.caption.equalsTo(b.caption)
    }
    MessageLocation.CONSTRUCTOR -> {
      require(this is MessageLocation && b is MessageLocation)
      this.equalsTo(b)
    }
    MessagePhoto.CONSTRUCTOR -> {
      require(this is MessagePhoto && b is MessagePhoto)
      this.photo.equalsTo(b.photo) && this.caption.equalsTo(b.caption)
    }
    MessageSticker.CONSTRUCTOR -> {
      require(this is MessageSticker && b is MessageSticker)
      this.sticker.equalsTo(b.sticker)
    }
    MessageVenue.CONSTRUCTOR -> {
      require(this is MessageVenue && b is MessageVenue)
      this.venue.equalsTo(b.venue)
    }
    MessageVideo.CONSTRUCTOR -> {
      require(this is MessageVideo && b is MessageVideo)
      this.video.equalsTo(b.video) && this.caption.equalsTo(b.caption)
    }
    MessageAudio.CONSTRUCTOR -> {
      require(this is MessageAudio && b is MessageAudio)
      this.audio.equalsTo(b.audio) && this.caption.equalsTo(b.caption)
    }
    MessageDocument.CONSTRUCTOR -> {
      require(this is MessageDocument && b is MessageDocument)
      this.document.equalsTo(b.document) && b.caption.equalsTo(b.caption)
    }
    else -> {
