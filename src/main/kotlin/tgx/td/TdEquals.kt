...
fun PollOption.equalsTo(b: PollOption, onlyTextContent: Boolean = false): Boolean {
  ...
  return this.text.equalsTo(b.text) && (onlyTextContent || (
    ...
    this.recentVoterIds.equalsTo(b.recentVoterIds) &&
    this.isBeingChosen == b.isBeingChosen &&
    this.author.equalsTo(b.author) &&
    this.additionDate == b.additionDate &&
    this.media.contentEquals(b.media)
  ))
}
...
fun PollType?.equalsTo(other: PollType?): Boolean = this.safeEqualsTo(other) { b ->
  when (this.constructor) {
    ...
    PollTypeQuiz.CONSTRUCTOR -> {
      ...
      this.correctOptionIds.contentEqualsOrEmpty(b.correctOptionIds) &&
      this.explanationMedia.contentEquals(b.explanationMedia)
    }
    else -> {
...
@OptIn(ExperimentalContracts::class)
fun PollMedia?.contentEquals(other: PollMedia?): Boolean = this.safeEqualsTo(other) { b ->
  when (this.constructor) {
    PollMediaAnimation.CONSTRUCTOR -> {
      require(this is PollMediaAnimation && b is PollMediaAnimation)
      if (COMPILE_CHECK) {
        PollMediaAnimation(
          this.animation
        )
      }
      this.animation.animation.equalsTo(b.animation.animation)
    }
    PollMediaAudio.CONSTRUCTOR -> {
      require(this is PollMediaAudio && b is PollMediaAudio)
      if (COMPILE_CHECK) {
        PollMediaAudio(
          this.audio
        )
    ...
    PollMediaDocument.CONSTRUCTOR -> {
      require(this is PollMediaDocument && b is PollMediaDocument)
      if (COMPILE_CHECK) {
        PollMediaDocument(
          this.document
        )
    ...
    PollMediaLink.CONSTRUCTOR -> {
      require(this is PollMediaLink && b is PollMediaLink)
      if (COMPILE_CHECK) {
        PollMediaLink(
          this.url,
          this.linkPreview
      ...
      this.url == b.url && this.linkPreview.equalsTo(b.linkPreview)
    }
    PollMediaLocation.CONSTRUCTOR -> {
      require(this is PollMediaLocation && b is PollMediaLocation)
      if (COMPILE_CHECK) {
        PollMediaLocation(
          this.location
        )
      }
      this.location.equalsTo(b.location)
    }
    PollMediaPhoto.CONSTRUCTOR -> {
      require(this is PollMediaPhoto && b is PollMediaPhoto)
      if (COMPILE_CHECK) {
        PollMediaPhoto(
          this.photo,
          this.video
      ...
      this.photo.equalsTo(b.photo) && this.video.equalsTo(b.video)
    }
    PollMediaSticker.CONSTRUCTOR -> {
      require(this is PollMediaSticker && b is PollMediaSticker)
      if (COMPILE_CHECK) {
        PollMediaSticker(
          this.sticker
        )
    ...
    PollMediaVenue.CONSTRUCTOR -> {
      require(this is PollMediaVenue && b is PollMediaVenue)
      if (COMPILE_CHECK) {
        PollMediaVenue(
          this.venue
        )
      }
      this.venue.equalsTo(b.venue)
    }
    PollMediaVideo.CONSTRUCTOR -> {
      require(this is PollMediaVideo && b is PollMediaVideo)
      if (COMPILE_CHECK) {
        PollMediaVideo(
          this.video,
      ...
      this.video.equalsTo(b.video) && this.cover.equalsTo(b.cover) &&
      this.startTimestamp == b.startTimestamp
      // TODO: alternativeVideos, storyboards
    }
    else -> {
      assertPollMedia_87b6242c()
      false
    }
  }
}
...
fun Poll.equalsTo(b: Poll, onlyTextContent: Boolean = false): Boolean {
  ...
  return this.question.equalsTo(b.question) &&
    ...
    this.canSeeResults == b.canSeeResults &&
    this.isAnonymous == b.isAnonymous &&
    this.allowsMultipleAnswers == b.allowsMultipleAnswers &&
    this.allowsRevoting == b.allowsRevoting &&
    this.optionOrder.contentEquals(b.optionOrder) &&
...
fun ChatEventLogFilters?.equalsTo(b: ChatEventLogFilters?): Boolean {
  ...
  return (
    ...
    (this?.memberTagChanges ?: false) == (b?.memberTagChanges ?: false) &&
...
fun DraftMessage?.equalsTo(b: DraftMessage?, ignoreDate: Boolean = false): Boolean {
  return when {
    ...
    this == null || b == null -> false
      ...
      (this.date == b.date || ignoreDate) && this.replyTo.equalsTo(b.replyTo) && this.content.equalsTo(b.content)
    }
  }
}
